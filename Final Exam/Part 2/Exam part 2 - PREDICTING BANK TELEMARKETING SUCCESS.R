...
log = glm(y~.-duration-euribor3m-nr.employed, data=train, family="binomial")
...
pred.log.test = predict(log, newdata=test, type="response")

#Then, using a probability threshold of 0.5, create a confusion matrix for
#the test set.
table(test$y, pred.log.test>0.5)
...
library(ROCR)
ROCRpred.log = prediction(pred.log.test, test$y)
...
library(rpart)
library(rpart.plot)
tree = rpart(y~.-duration-euribor3m-nr.employed, data=train, method="class", cp=0.012)
prp(tree)
##Answer: emp.var.rate
...
pred.tree.test = predict(tree, newdata=test, type="class")
